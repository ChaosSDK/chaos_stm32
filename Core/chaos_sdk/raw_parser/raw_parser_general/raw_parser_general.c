...
#   ifdef D_RAW_P_REED_SOLOMON_ECC_CORR_ENA
            ...
            rscode_decode(&self->rs_ecc, descriptor->RX_buf, rx_pos);
            rx_pos -= RSCODE_NPAR;
#   endif /* D_RAW_P_REED_SOLOMON_ECC_CORR_ENA */
...
void RawParser_gen_send_message(RawParser_gen_t* const self, const u8* const data, const reg size, PREPROCESSOR_CTX_TYPE(ctx))
{
    ...
    M_Assert_BreakSaveCheck((size > (D_RAW_P_CHECK_LEN - 1U - sizeof(rawP_crc_t) - RSCODE_NPAR)), M_EMPTY, return, "RawParser_gen_send_message: No valid input size");
    ...
    M_Assert_BreakSaveCheck((size > (D_RAW_P_CHECK_LEN - 1U - RSCODE_NPAR)), M_EMPTY, return, "RawParser_gen_send_message: No valid input size");
#else
...
#if defined(D_RAW_P_REED_SOLOMON_ECC_CORR_ENA)
    reg tmp_len = (size + RSCODE_NPAR);
#else
    reg tmp_len = size;
#endif /* defined(D_RAW_P_REED_SOLOMON_ECC_CORR_ENA) */
